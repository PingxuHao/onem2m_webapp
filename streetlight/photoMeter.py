...
def createAE(resourceName):
    ...
    payld = { "m2m:ae": { "rr": True, "api": "NR_AE001", "apn": "IOTApp", "csz": [ "application/json" ], "srv": [ "2a" ], "rn": resourceName, "poa": [ '123s' ],"acpi": ["cse-in"] } }
...
# This is GUI logic - not oneM2M logic
# Lookup dictionary that maps events (button and from threads) to function to call
dispatch_dictionary = {'1':button1,
                       '2':button2,
                       '3':button3,
                       '-Phosphorescence-':button3,
                       '4':button4,
                       '5':button5,
                       '-Lamp-':lampCommandHandler,
                       'Register':buttonReg,
                       'Deregister':buttonDeReg}

# sensor structure will be a list column with 3 element across each row in the column
sensor1 = [
        [
            sg.Button(image_data=convert_to_bytes("streetLightBlue.png", (40, 40)), button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0, key='1'),
            sg.Text("Illuminance"),
            sg.Input("34", size=(15,1), enable_events=False, key="-Illuminance-"), # parameterize initial value
        ]
    ]

sensor2 = [
        [
            sg.Button(image_data=convert_to_bytes("streetLightBlue.png", (40, 40)), button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0, key='2'),
            sg.Text("Luminescence"),
            sg.Input("ds", size=(15,1), enable_events=False, key="-Luminescence-"),
        ]
    ]

sensor3 = [
        [
            sg.Button(image_data=convert_to_bytes("streetLightBlue.png", (40, 40)), button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0, key='3'),
            sg.Text("Phosphorescence"),
            sg.Input(size=(15,1), enable_events=False, key="-Phosphorescence-"),
        ]
    ]

sensor4 = [
        [
            sg.Button(image_data=convert_to_bytes("streetLightBlue.png", (40, 40)), button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0, key='4'),
            sg.Text("LightAbsorption"),
            sg.Input(size=(15,1), enable_events=False, key="-LightAbsorption-")
        ]
    ]

lamp1 = [ #No input field for the lamp. clicking the button generates the input
        [
            sg.Button(image_data=convert_to_bytes("streetLightBlue.png", (40, 40)), button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0, key='5'),
            sg.Text("Lamp")
        ]
    ]

device = [sg.Button("Register"), sg.Button("Deregister"), sg.Exit()]

device_layout = [
    [collapse(sensor1,'-ILLUMINANCE-')],
    [collapse(sensor2,'-LUMINESCENCE-')],
    [collapse(sensor3,'-PHOSPHORESCENCE-')],
    [collapse(sensor4,'-LIGHTABSORPTION-')],
    [collapse(lamp1,'-LAMP1-')],
    [device]
]

 

# Create the window for the device simulator. fields will be hidden based on configuration
window = sg.Window(deviceName, device_layout, location=(X_position,Y_position), finalize=True) # parameterize "Demo" and location
if Illuminance != "YES":
    window['-ILLUMINANCE-'].update(visible=False)
if Luminescence != "YES":
    window['-LUMINESCENCE-'].update(visible=False)
if Phosphorescence != "YES":
    window['-PHOSPHORESCENCE-'].update(visible=False)
if LightAbsorption != "YES":
    window['-LIGHTABSORPTION-'].update(visible=False)
if Lamp1 != "YES":
    window['-LAMP1-'].update(visible=False)

if  REGISTER_ON_START == "YES":
            # register function
            print("Registering")

# Start the thread for notifications
threading.Thread(target=lamp_command, args=(window,), daemon=True).start()
